...
void GOPortal::setupPortal(float x1, float y1, float x2, float y2) {
	if (x1 > x2) {
		float temp = x1;
		x1 = x2;
		x2 = temp;
	...
	GOPortal* portal1 = new GOPortal(x1, y1, 3.0f);
	GOPortal* portal2 = new GOPortal(x2, y2, -3.0f);
...
GOPortal::GOPortal(float x, float y, float direction) :
	GOBlock(x, y, 0.5f, 5.5f, 0.0f, "WallBall/media/color3.bmp"),
